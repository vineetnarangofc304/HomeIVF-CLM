...
- **Hardening this iteration (`backend/routes/whatsapp.py`):** the only two UNCAPPED WhatsApp reads — `list_channels` and `channel_messages` — now use `.max_time_ms(5000)` + try/except PyMongoError → 503 (items) and `count_documents(maxTimeMS=4000)` → total=-1 (graceful), matching the hot pollers. So they abort at 5s and release their pooled connection instead of hanging. Hot pollers (/calls/active, /followups/reminders, /unread-summary) were already fail-fast.
- **Verified:** testing_agent iter75 = backend 13/13, frontend 100%, retest not needed. Inbox list (all filters + pagination + search), a conversation's messages (order/pagination/search), unread-summary, caller scoping (owner_id), and the WhatsApp UI tab all work; hot-poll regressions 200.
- **⚠️ NEEDS PRODUCTION REDEPLOY** (brings the iter74 leads fix + db.py pool timeouts + this hardening — together these end the 180s cascade).
- **Noted (future, low-risk RBAC):** /channels/{id}/messages doesn't re-verify the channel belongs to the caller (relies on list scoping to keep ids private). Not part of this ask.


## P0 (2026-07-25a) — Filtered Leads (Tag + Lead-stage) → 18s 504 → pool-exhaustion CASCADE — ROOT-CAUSED & FIXED, bug_testing_agent-verified (iter74), needs REDEPLOY
- **User report (production):** on 'Leads in Pipeline (All)' add Tag='OPD Booked' + Lead stage='Contacted' → ~30s → "Server is busy". System Health showed `GET /api/leads` = **504 @ ~18s** (many callers) cascading into **180s** hangs on `/whatsapp/unread-summary` & `/calls/active` and **503** on `/reports/dashboard` = classic DB connection-pool exhaustion triggered by the one slow filtered query.
- **ROOT CAUSE:** `tags` is a **MULTIKEY** field. With a tags filter + `create_date` sort, the planner chose the plain `tags` index and did a **BLOCKING in-memory SORT** of the whole tag set → ~18s on the loaded prod DB → hit the list's 18s wall-clock → 504, while holding pooled connections → cascade. (The `hint` only covered the unscoped default, not filtered queries.)
- **Fix:** (1) NEW index `backend/server.py` INDEX_SPECS: `[active,lead_stage,tags,create_date,id]`. (2) `backend/routes/leads.py` list_leads: pin it BY KEY PATTERN for **admin/all-scope** (`user_id` absent) `single lead_stage + tags` create_date queries → single-tag IXSCAN / multi-tag SORT_MERGE, **NO blocking sort**, ~limit keys (not the 24k bucket). Also pin `[active,lead_stage,create_date,id]` for lead_stage-only. Caller-scoped queries intentionally skip the hint (their selective `user_id` index is better). (3) Fixed my earlier fallback: retry-without-hint now runs ONLY on a hint error (not a Mongo maxTimeMS timeout, `OperationFailure` code 50) so a slow query never runs twice.
- **PROVEN (explain):** single-tag = [LIMIT,FETCH,IXSCAN] SORT=False; multi-tag = [...,SORT_MERGE,IXSCAN×3] SORT=False.
- **Verified:** bug_testing_agent iter74 = backend 100% / frontend 100%, retest not needed. Full admin filter/sort/pagination matrix = 200 (no 504); caller scope stays correct; ascending fallback = 200; **34-request concurrency probe (24 filtered /api/leads + whatsapp/calls/dashboard) all 200 in ~1.9s, no cascade**; lead detail opens.
- **⚠️ NEEDS PRODUCTION REDEPLOY.** New index builds automatically on startup (confirmed in preview).


## P0 (2026-07-24d) — CONSTANT "Server is busy" on the Leads page ONLY, since last deploy — ROOT-CAUSED & FIXED, bug_testing_agent-verified (iter73), needs REDEPLOY
- **User report (production):** since the last deployment, the Leads page shows "Server is busy" CONSTANTLY (before it worked sometimes / sometimes a Cloudflare error). Every OTHER page (Follow-ups, Call Center, etc.) opens fine — only Leads fails.
- **TRUE ROOT CAUSE (proven):** `list_leads` pinned the sort index with `cur.hint("active_createdate_id")` — hinting by index **NAME**. But `server.py` `INDEX_SPECS` creates that index with **NO explicit name**, so on a fresh/production DB it is auto-named `active_1_create_date_-1_id_-1`. Preview coincidentally has a *historical* index literally named `active_createdate_id`, so the name-hint worked in preview but **threw a query error on EVERY unscoped 'Leads in Pipeline (All)' request on production → 504 → constant "Server is busy"** (admin default hits this branch; other pages don't). This is why preview never reproduced it and why it was Leads-only + constant.
- **PROVEN in preview:** simulated prod by recreating the index under the auto name — hint-by-NAME **FAILED**, hint-by-KEY-PATTERN **succeeded** (state restored after).
- **Fix (`backend/routes/leads.py`):** (1) hint by **KEY PATTERN** `[("active",1),("create_date",-1),("id",-1)]` instead of by name — resolves to the index regardless of its name (works on preview AND prod). (2) wrapped the find in `_run(with_hint)` with a **fallback**: if the hinted find raises PyMongoError, retry once WITHOUT the hint before 504-ing, so a hint/index issue can never take down the Leads page again.
- **Also shipped this session (both help Leads open reliably):** count **decoupled** from the list (`_cached_count` returns cached-or-(-1)+background compute on db_analytics → list opens in ~find time, never gated on the ~120k count; frontend renders -1 as "Many"); LeadDetail **secondary-read decoupling** (core `/leads/{id}` renders first; messages/whatsapp/calls/etc. load fault-tolerantly) — iter71.
- **Verified:** bug_testing_agent iter73 = backend 100% / frontend 100%, retest not needed. Admin/caller list all scopes/filters/sorts/pages → 200 with rows, no 'Server is busy'; first load total=-1 (~53ms) then cached 119813; lead detail opens even with secondaries forced to 504.
- **⚠️ NEEDS PRODUCTION REDEPLOY** — this is the fix for the constant Leads 'Server is busy'.
