...
async def export_leads_xlsx(params: dict = Depends(query_params_dep), user: dict = Depends(require_permission("export"))):
    ...
    async for lead in cursor:
        ...
        row = {
            **lead,
            # Business rule: a lead with NO disposition tag is always the "New" stage —
            # this keeps the export consistent with the CRM rule (a stage set without a
            # tag, e.g. a stray "Contacted", is normalised to "New" in the export).
            "lead_stage": (lead.get("lead_stage") or "New") if (lead.get("tags") or []) else "New",
